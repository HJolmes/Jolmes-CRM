...
async function main() {
  ...
  if (!filePath) {
    console.error('Usage: npx ts-node --esm prisma/import-offers-gsg.ts <path>')
    process.exit(1)
  }
  ...
  for (const row of rows) {
    const kundeRaw = String(row['Kunde_Komplett'] || '').trim()
    ...
    try {
      await prisma.offer.create({
        data: {
          ...
          sparte: 'GLAS_SONDER_GARTEN',
          angebotsNummer: String(row['Angebot-Nr.'] || '').trim() || null,
          ...
          rechnungNummer: String(row['RG-Nr.'] || '').trim() || null,
          rechnungSumme: parseFloat2(row['RG/ Summe,netto']),
          rechnungDatum: parseDate(row['RG Datum']),
          rgBezahlt: String(row['RG bezahlt?'] || '').toLowerCase() === 'ja',
